...
Configurando para por 80 - (Meu domínio: blog.seu-dominio.com.br)

```sh
sudo nano /etc/nginx/sites-available/blog.seu-dominio.com.br
```

Edita os dados abaixo:\
`blog.seu-dominio.com.br`\
Se você mudou a porta da aplicação, mude `3000` para o número que escolheu.
Também ajuste os caminhos `/home/seu-usuario/blog/public/` e
`/home/seu-usuario/blog/public/uploads/`

```
server {
  listen 80;
  server_name blog.seu-dominio.com.br;
  ...
  location /public/ {
    alias /home/seu-usuario/blog/public/;
  }

  # Servir arquivos estáticos do /public
  location /uploads/ {
    alias /home/seu-usuario/blog/public/uploads/;
  }
...
sudo ln -s /etc/nginx/sites-available/blog.rlimasistemas.com.br /etc/nginx/sites-enabled/
...
sudo certbot --nginx -d blog.rlimasistemas.com.br
...
sudo chmod o+x /home/seu-usuario
sudo chmod o+x /home/seu-usuario/blog
sudo chmod o+x /home/seu-usuario/blog/public
# só aqui fiz recursão para evitar mudar todos os arquivos do projeto
sudo chmod -R o+rx /home/seu-usuario/blog/public/uploads
```
...
# Ver
pm2 list # listar
pm2 restart blog # reinicia
pm2 stop blog # para
pm2 start blog # inicia
pm2 log blog # veja o log do next (importante para debug)
...
pm2 delete blog # reinicia
pm2 start npm --name blog -- start -i max # modo cluster, 1 instância por core do processador
pm2 save
```
...
server {
  ...
  ssl_certificate /etc/letsencrypt/live/blog.seu-dominio.com.br/fullchain.pem; # managed by Certbot
  ssl_certificate_key /etc/letsencrypt/live/blog.seu-dominio.com.br/privkey.pem; # managed by Certbot
...
server {
  ...
  if ($host = blog.seu-dominio.com.br) {
...
echo "cd /home/seu/blog"
cd /home/seu-usuario/blog
echo
...
echo "Executando: pm2 restart blog"
pm2 restart blog
echo
...
```sh
ssh seu-usuario@blog.seu-dominio.com.br './refresh-blog.sh'
```
